...
def mainlist(item):
    logger.info("tvalacarta.channels.rtve_api mainlist")

    itemlist = []
    itemlist.append( Item(channel=__channel__, title="Por cadenas" , url="http://www.rtve.es/api/cadenas.json?size=60" , action="cadenas" , folder=True) )
    itemlist.append( Item(channel=__channel__, title="Todos" , url="http://www.rtve.es/api/programas.json?size=60" , action="programas" , folder=True) )
    return itemlist

def cadenas(item):
    logger.info("tvalacarta.channels.rtve_api cadenas")
    ...
    for json_item in json_items:
        ...
        url = json_item["programsRef"]+".json?size=60"
        thumbnail = ""
        plot = ""
        fanart = ""
        page = json_item["htmlUrl"]
        if (DEBUG): logger.info(" title=["+repr(title)+"], url=["+repr(url)+"], thumbnail=["+repr(thumbnail)+"] plot=["+repr(plot)+"]")
        itemlist.append( Item(channel=__channel__, title=title , action="programas" , url=url, thumbnail=thumbnail, plot=plot , page=page, show=title , fanart=fanart, folder=True) )

def programas(item, load_all_pages=False):
    logger.info("tvalacarta.channels.rtve_api programas")
